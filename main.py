...
def handle_status(chat_id):
    ...
    tg(
        ...
        f"Open positions: {len(_positions)}/{os.getenv('MAX_OPEN_POSITIONS','3')}\n"
...
def handle_balance(chat_id):
    try:
        balance = float(get_clob_client().get_balance())
...
def handle_report(chat_id):
    ...
    tg(
        ...
        f"🔄 Total trades: {_trade_count}\n"
        f"⚡ Trades/hr: {tph:.1f}\n"
        f"📂 Open positions: {len(_positions)}\n"
        f"💵 Daily PnL: ${_daily_pnl:.4f}\n"
...
def get_clob_client():
    ...
    if _clob_client is None:
        ...
        _clob_client = ClobClient(host=CLOB_API, key=POLYMARKET_PK, chain_id=POLYGON, signature_type=0, funder=POLYMARKET_FUNDER)
...
def bayesian_analyze(yes_price, ob_imbalance, trade_direction, spread, volume_24h):
    ...
    likelihood   = float(ob_imbalance)*0.30 + ((float(trade_direction)+1)/2)*0.35 + max(0.1,1.0-(float(spread)/0.10))*0.15 + (1/(1+np.exp(-(float(volume_24h)-50000)/20000)))*0.20
    ...
    kelly = max(0.0, min(((trade_prob*(1-ref_price)/ref_price-(1-trade_prob))/((1-ref_price)/ref_price))*KELLY_FRACTION, 0.20)) if 0 < ref_price < 1 else 0.0
    return {"posterior": round(posterior,4), "edge": round(edge,4), "confidence": round(confidence,4),
            "kelly": round(kelly,4), "direction": direction,
...
def tool_scan_markets(min_volume=5000, limit=20):
    try:
        r = requests.get(f"{GAMMA_API}/markets",
            params={"active":"true","closed":"false","limit":limit,"order":"volume24hr","ascending":"false"}, timeout=15)
        r.raise_for_status()
        data = r.json()
        ...
        for m in markets:
            ...
            yes_tid = no_tid = ""
            for t in tokens:
                if t.get("outcome") == "Yes": yes_price = float(t.get("price") or 0.5); yes_tid = t.get("token_id","")
                elif t.get("outcome") == "No": no_price = float(t.get("price") or 0.5); no_tid = t.get("token_id","")
            result.append({"market_id":m.get("conditionId",""),"question":m.get("question","")[:80],
                "yes_price":round(yes_price,3),"no_price":round(no_price,3),"yes_token_id":yes_tid,"no_token_id":no_tid,"volume_24h":vol})
    ...
    try:
        ...
        if yes_token_id:
            try:
                ...
                bid_vol = sum(float(b.get("size",0)) for b in bids); ask_vol = sum(float(a.get("size",0)) for a in asks)
                ob_imbalance = bid_vol/(bid_vol+ask_vol) if (bid_vol+ask_vol) > 0 else 0.5
        ...
        result["recommendation"] = f"{'✅ TRADE' if result['should_trade'] else '❌ SKIP'} {result['direction']} @ {yes_price:.3f} | edge={result['edge']:+.3f} conf={result['confidence']:.3f}"
...
def tool_place_order(market_id, token_id, direction, price, kelly_fraction):
    ...
    try:
        if _paused: return json.dumps({"status":"skipped","reason":"agent_paused"})
        if len(_positions) >= int(os.getenv("MAX_OPEN_POSITIONS","3")): return json.dumps({"status":"skipped","reason":"max_positions_reached"})
        client = get_clob_client()
        balance = float(client.get_balance())
        if balance < 1.0: return json.dumps({"status":"skipped","reason":f"balance_too_low:${balance:.2f}"})
        usdc_bet = min(float(kelly_fraction)*balance, MAX_POSITION_USDC)
        price = float(price)
        if not (0 < price < 1): return json.dumps({"status":"error","message":f"invalid_price:{price}"})
        size = round(usdc_bet/price, 2)
        if size < 1.0: return json.dumps({"status":"skipped","reason":f"size_too_small:{size}"})
        ...
        _trade_count += 1
        tg(f"🟢 <b>ORDER PLACED</b>\nMarket: <code>{market_id[:12]}</code>\nDirection: <b>{direction}</b>\nSize: {size} @ ${price:.3f}\nCost: ${size*price:.2f} USDC\nOrder: <code>{order_id}</code>")
        return json.dumps({"status":"ok","order_id":order_id,"direction":direction,"size":size,"price":price,"usdc_cost":round(size*price,2)})
    except Exception as e:
...
def tool_check_positions():
    ...
    tph = (_trade_count / max(elapsed,1)) * 3600
    return json.dumps({
        "status":"ok","open_positions":len(_positions),"max_positions":int(os.getenv("MAX_OPEN_POSITIONS","3")),
        "total_trades":_trade_count,"trades_per_hour":round(tph,1),"daily_pnl_usdc":round(_daily_pnl,4),
        "daily_loss_limit":MAX_DAILY_LOSS_USDC,"daily_loss_remaining":round(MAX_DAILY_LOSS_USDC+_daily_pnl,2),
        "uptime_minutes":round(elapsed/60,1),"paused":_paused,
        "positions":[{"order_id":oid[:12],"market_id":p["market_id"][:12],"direction":p["direction"],
            "size":p["size"],"entry_price":p["entry_price"],"usdc_cost":p["usdc_cost"]} for oid,p in _positions.items()]
    })
...
def tool_cancel_order(order_id):
    global _positions
    try:
        get_clob_client().cancel(order_id); _positions.pop(order_id, None)
...
TOOL_SCHEMAS = [
    {"type":"function","function":{"name":"scan_markets","description":"Scan Polymarket active markets by volume. Call first every cycle.",
        "parameters":{"type":"object","properties":{"min_volume":{"type":"number"},"limit":{"type":"integer"}},"required":[]}}},
    {"type":"function","function":{"name":"analyze_market","description":"Bayesian analysis. Returns edge, confidence, direction, kelly, should_trade.",
    ...
    {"type":"function","function":{"name":"place_order","description":"Place BUY order. ONLY when should_trade=true AND edge>=0.03 AND confidence>=0.60.",
    ...
    {"type":"function","function":{"name":"check_positions","description":"Check open positions, P&L, risk. Call every cycle.",
        "parameters":{"type":"object","properties":{},"required":[]}}},
    {"type":"function","function":{"name":"cancel_order","description":"Cancel specific order.",
        "parameters":{"type":"object","properties":{"order_id":{"type":"string"}},"required":["order_id"]}}},
    {"type":"function","function":{"name":"cancel_all_orders","description":"Emergency cancel ALL orders.",
        "parameters":{"type":"object","properties":{},"required":[]}}},
]

TOOL_MAP = {"scan_markets":tool_scan_markets,"analyze_market":tool_analyze_market,"place_order":tool_place_order,
    "check_positions":tool_check_positions,"cancel_order":tool_cancel_order,"cancel_all_orders":tool_cancel_all_orders}
...
def run_agent_cycle(client, cycle):
    ...
    while True:
        step += 1
        if step > 25: break
        response = client.chat.completions.create(model=GROQ_MODEL, messages=messages, tools=TOOL_SCHEMAS, tool_choice="auto", temperature=0.1, max_tokens=4096)
...
def main():
    ...
    print("="*50)
    if not GROQ_API_KEY: print("❌ Missing GROQ_API_KEY"); return
    if not POLYMARKET_PK or not POLYMARKET_FUNDER: print("❌ Missing POLYMARKET keys"); return
    ...
    while True:
        cycle += 1
        print(f"\n━━━━━ Cycle #{cycle} | {datetime.now().strftime('%H:%M:%S')} | Paused:{_paused} ━━━━━")
        try:
            if not _paused:
